...
AIX Skills 是一个公开的 Agent Skills 集合，提供可直接使用的 skills，以及创建 skills 的模板和规范。

本仓库采用混合型结构：一部分是自研 skills，一部分是模板和规范。目标是让读者既能拿来用，也能照着规范贡献新的 skill。
...
| `battery-charger-fuel-gauge-integration` | 帮助 agent 通过 I2C 集成与排查锂电池充电 IC 与电量计(BQ24295、BQ27441、MAX17048):输入限流、看门狗 host 模式、CC/CV、NTC/JEITA 保护和 SOC profile 校准。 |
...
| `cellular-at-modem-integration` | 帮助 agent 通过 UART 集成和调试 Quectel、SIMCom、u-blox 蜂窝 AT 模组,涵盖网络注册、APN/PDP 上下文、按模组系列区分的 TCP/UDP socket 栈(Quectel 用 QIOPEN,SIMCom 用 CIPSTART 或 NETOPEN/CIPOPEN,u-blox 用 USOCx)、URC 解析和 PSM/eDRX 省电。 |
...
| `gnss-gps-integration` | 帮助 agent 通过 UART 集成 GNSS/GPS 模组(u-blox NEO-M8N/M9N、MAX-M10,Quectel L76/L80/LC76),解析 NMEA GGA/RMC/GSA/GSV 与 UBX-NAV-PVT,排查无法定位、TTFF、校验和及 PPS 问题。 |
...
| `tflite-micro-integration` | 帮助 agent 在 MCU 上集成 TensorFlow Lite Micro(LiteRT for Microcontrollers)—— MicroInterpreter、算子注册、tensor arena 规划、int8 量化和推理调用。 |
...
本仓库本身就是一个 [Claude Code 插件市场（plugin marketplace）](https://code.claude.com/docs/en/plugin-marketplaces)。添加一次市场，即可安装全部 skills：
...
只想要某一个 skill，或使用其它 agent 运行环境？直接把目录复制到你的 skills 目录：

```bash
cp -R skills/webpage-to-markdown ~/.claude/skills/
```

个人级 skill 放 `~/.claude/skills/`，项目级 skill 放 `<仓库>/.claude/skills/`。
...
编辑 `skills/my-new-skill/SKILL.md` 后运行校验（需要 PyYAML）：
